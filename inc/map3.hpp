...
/*   Updated: 2023/02/16 17:53:49 by chduong          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft 
{
	...
	class map {
		public:
			...
			typedef RBTree<key_type, value_type, value_compare, node_allocator>			tree_type;
		...
		public:
			// ====================== Member Functions ========================= //
			explicit map(const Compare& comp = key_compare(), const Alloc& alloc = Alloc())	  : _tree(tree_type(value_compare(comp), node_allocator(alloc))) {};
			...
			: _tree(tree_type(value_compare(comp), node_allocator(alloc))) {this->insert(first, last);}

			map(const map &x) : _tree(x._tree) {insert(x.begin(), x.end());}
			...
			allocator_type				get_allocator() const {return _tree.getAlloc();}
